...
(define (reduce . args) (apply apply-generic (cons 'reduce args)))
...
(define (install-polynomial-package)
  ...
  (define (pseudo-remainder-terms a b)
    (let ((o1 (order (first-term a)))
      ...
      (remainder-terms (mul-termlist-by-term (make-term 0
                                                        (pow c (+ 1 (- o1 o2))))
                                             a)
                       b)))
  (define (cut-odds-termlist L)
    (define (gcd-coeff L)
      (if (empty-termlist? L)
          0
          (gcd (coeff (first-term L)) (gcd-coeff (rest-terms L)))))
    (div-terms-by-num L (gcd-coeff L)))
  (define (gcd-terms a b)
    (if (empty-termlist? b)
        a
        (cut-odds-termlist (gcd-terms b (pseudo-remainder-terms a b)))))
  (define (reduce-terms a b)
    (let ((g (gcd-terms a b)))
      (list (car (div-terms a g))
            (car (div-terms b g)))))
  ...
  (define (reduce-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((term-res (reduce-terms (term-list p1)
                                      (term-list p2))))
...
(define p1 (make-poly 'x '(sparse (term 1 1) (term 0 1))))
(define p2 (make-poly 'x '(sparse (term 3 1) (term 0 -1))))
(define p3 (make-poly 'x '(sparse (term 1 1))))
(define p4 (make-poly 'x '(sparse (term 2 1) (term 0 -1))))
