      ...
      (cond (proc (apply proc (map contents args))) ; base case, no coercions needed
            ...
            (else
             (let ((htag (highest-tag (unique data-tags)))) ; get tag with gighest level in tag tower from unique data-tags
               (let ((proc (get op (map (lambda (x) htag) args)))) ; get procedure to arguments with htag tah
                 (if proc
                     (apply proc (map contents ; get content of args
                                      (map (lambda (x) (raise-to htag x)) ; get raised to htag args
                                           args)))
                     (raise-exception)))))))))
